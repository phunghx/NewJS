...
learner = TRPO(args, [55+7], [18], learner_tasks, learner_results)
learner.start()
...
while True:
    ...
    if iteration % 100 == 0:
        history["weight"].append(new_policy_weights)
        with open("./logs/%s-%s-%f-%f-%f-%f" % (args.task, args.decay_method, starting_timesteps, starting_kl, args.timestep_adapt, args.kl_adapt), "w") as outfile:
